defmodule LingoManagerWeb.CoreComponents do
  @moduledoc """
  ...
  import LingoManagerWeb.Gettext
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LingoManagerWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LingoManagerWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  attr :class, :string, default: "mt-6"
    ...
    <div class={["flex items-center justify-between", @class]}>
      ...
      <nav class="flex items-center space-x-2" aria-label="Pagination">
        <%= if @pagination.has_prev do %>
          <.link navigate={"#{@base_path}?page=#{@pagination.page - 1}"}
        ...
        <%= for page_num <- max(1, @pagination.page - 2)..min(@pagination.total_pages, @pagination.page + 2) do %>
          ...
          <% else %>
            <.link navigate={"#{@base_path}?page=#{page_num}"}
